...
const tienePermisosUsuario = async (idUsuario, usuario) => {
  const { role, id } = usuario;
  const usu = await Usuarios.findById(idUsuario);
  if (usu.id.toString() === id || role === 'ADMIN_ROLE') {
    return true;
  } else {
    return false;
  }
...
module.exports = {
  ...
  tienePermisosProducto,
  tienePermisosUsuario
};
